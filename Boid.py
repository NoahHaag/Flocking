...
class Boid(pg.sprite.Sprite):
    ...
    def __init__(self, grid, drawSurf):  #, cHSV=None
        ...
        self.color.hsva = popOneColor #if cHSV is None else cHSV # randint(5,55) #4goldfish
...
class Rocks(pg.sprite.Sprite):
    def __init__(self, grid, drawSurf):  #, cHSV=None
        ...
        self.image.set_colorkey(0)
        self.color = pg.Color(0)  # preps color so we can use hsva
        self.color.hsva = popTwoColor #if cHSV is None else cHSV # randint(5,55) #4goldfish
        pg.draw.polygon(self.image, self.color, ((7,0),(12,5),(3,14),(11,14),(2,5),(7,0)), width=3)
        self.image = pg.transform.scale(self.image, (16, 24))
        self.bSize = 17
        self.orig_image = pg.transform.rotate(self.image.copy(), -90)
    ...
    def update(self, dt, speed, ejWrap=False):
        ...
        if self.grid_pos != self.grid_lastpos:
            ...
            self.grid_lastpos = self.grid_pos
        # get nearby boids and sort by distance
        near_rocks = self.grid.getnear(self, self.grid_pos)
        neirocks = sorted(near_rocks, key=lambda i: pg.Vector2(i.rect.center).distance_to(selfCenter))
        del neirocks[7:]  # keep 7 closest, dump the rest
        # when boid has neighborS (walrus sets ncount)
        if (ncount := len(neirocks)) > 1:
            nearestRock = pg.Vector2(neirocks[0].rect.center)
            for nRocks in neirocks:  # adds up neighbor vectors & angles for averaging
                xvt += nRocks.rect.centerx
                yvt += nRocks.rect.centery
                yat += sin(radians(nRocks.ang))
                xat += cos(radians(nRocks.ang))
            tAvejAng = degrees(atan2(yat, xat))
            targetV = (xvt / ncount, yvt / ncount)
            # if too close, move away from closest neighbor
            if selfCenter.distance_to(nearestRock) < self.bSize: targetV = nearestRock
            tDiff = targetV - selfCenter  # get angle differences for steering
            tDistance, tAngle = pg.math.Vector2.as_polar(tDiff)
            # if boid is close enough to neighbors, match their average angle
            if tDistance < self.bSize * 5: tAngle = tAvejAng
            ...
            if abs(tAngle - self.ang) > .5: turnDir = (angleDiff / 360 - (angleDiff // 360)) * 360 - 180
            # if boid gets too close to target, steer away
            if tDistance < self.bSize and targetV == nearestRock: turnDir = -turnDir
        # Avoid edges of screen by turning toward the edge normal-angle
        sc_x, sc_y = self.rect.centerx, self.rect.centery
        if not ejWrap and min(sc_x, sc_y, maxW - sc_x, maxH - sc_y) < margin:
            if sc_x < margin:
...
class RockGrid() :  # tracks boids in spatial partition grid
...
def main():
    ...
    rockTracker = RockGrid()
    nBoids = pg.sprite.Group()
    # spawns desired # of boidz
    for n in range(BOIDZ): nBoids.add(Boid(boidTracker, screen))
    nRocks = pg.sprite.Group()
    for n in range(ROCKZ): nRocks.add(Rocks(rockTracker, screen))

    chased = pg.sprite.Group()
    ...
    while True:
        ...
        nBoids.draw(screen)
        nRocks.draw(screen)
        nRocks.update(dt, ROCKSPEED, WRAP)
        ...
        #pg.sprite.spritecollide(Boid(boidTracker, screen), nRocks, dokill=True)
